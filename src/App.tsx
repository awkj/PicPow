...
function App() {
  ...
  const { files, settings, setSettings, addFiles, downloadFile, downloadAll, retryFile, resetSettings } = useCompressor(
  ...
  return (
    <div className="min-h-screen hero-gradient flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl flex flex-col flex-1">
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-slate-700 dark:text-slate-200 font-medium text-lg leading-none">PicMan</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={`bg-transparent ${showSettings ? "text-slate-800 dark:text-white bg-slate-200/50 dark:bg-white/10" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5"}`}
              aria-label="设置"
            >
            ...
            <Button
              ...
              className="bg-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5"
              aria-label="切换主题"
            >
              {isDark ? (
                // 太阳图标
          ...
          <AnimatePresence>
            {!hasFiles && (
              ...
              >
                ...
                <p className="text-gray-500 dark:text-gray-500 text-xs">所有压缩在本地完成，图片不会上传到服务器</p>
              </motion.div>
            )}
          </AnimatePresence>
        </main>
      ...
      {/* ── 对比弹窗 ── */}
      <CompareModal file={previewFile} isOpen={isCompareOpen} onClose={() => setIsCompareOpen(false)} />
    </div>
  )
}
